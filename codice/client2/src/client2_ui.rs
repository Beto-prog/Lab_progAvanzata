    ...
    pub fn create_command(&mut self) -> String {
        ...
        if self.communication_server_commands.contains(&self.selected_command){
            if self.selected_command.eq("client_list?"){
                ...
                Client2_UI::create_message_for_command(&self.selected_command,self.selected_client_id,&self.input_text,self.selected_server.0)
            }
        }
        else {
            if self.selected_command.eq("files_list?"){
                ...
                Client2_UI::create_complex_command(&self.selected_command,&self.selected_content_id,self.selected_server.0)
            }
        }
    }
