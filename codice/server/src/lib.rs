...
impl Server {
    pub fn new(
        ...
        for (key, _) in packet_send.iter() {
            //start by filling the graph with the directly connected neighbour
    ...
    */
    ...
    fn handle_packet(&mut self, mut packet: Packet) {
        ...
        match packet.routing_header.hops.get(0) {
            //trying to get the source id
            None => match &packet.pack_type {
                PacketType::FloodRequest(_) => {
                    println!(
                        "{}",
                        "SERVER --> Received FloodRequest".white().bold().on_blue()
                    )
                }
                ...
                _ => {
                    println!(
                        "{}",
                        "SERVER -->  I received an packet without headers"
                            .white()
                            .bold()
                            .on_blue()
                    );
        ...
        match packet.pack_type {
            //Process different packet type
            PacketType::MsgFragment(msg) => {
                println!(
                    "{} {:?}",
                    "SERVER --> Recived message".white().bold().on_blue(),
                    msg
                );
                ...
                let result =
                    self.package_handler
                        .process_fragment(packet.session_id, source_id as u64, msg); //All the request send by the client are sort . I refuse to eleborate request longer than 128
                println!(
                    "{} {:?}",
                    "SERVER --> Request".white().bold().on_blue(),
                    result
                );
                ...
                match result {
                    Ok(Some(data)) => {
                        ...
                        let mut msg_work = "".to_string(); //temporary value for message
                        ...
                        match message {
                            Ok(m) => {
                                ...
                                println!(
                                    "{} {}",
                                    "SERVER --> Request converted ".white().bold().on_blue(),
                                    m
                                );
                            }
                            Err(m) => {
                        ...
                        //Process the request
                        let result = self.server_type.process_request(
                            msg_work.clone(),
                            source_id as u32,
                            &mut flag,
                        );
                        ...
                        if flag == 0 {
                            ...
                            */
                            if let Some(content) = msg_work
                                .strip_prefix("message_for?(")
                                .and_then(|s| s.strip_suffix(")"))
                            {
                                let parts: Vec<&str> = content.splitn(2, ',').collect();
                                if parts.len() == 2 {
                                    match parts[0].parse::<NodeId>() {
                                        Ok(v) => source_id = v,
                                        Err(_) => {
                                            println!("Server --> Unable to parse node id!");
                                        }
                                    }
                                }
                            }
                        ...
                        match result {
                            Ok(value) => {
                                ...
                                self.paket_ack_manger
                                    .insert((source_id, packet.session_id), value.clone());
                    ...
                    _ => {
                        println!(
                            "{}",
                            "Error fragment to long - refuse to process"
                                .white()
                                .bold()
                                .on_blue()
                        )
            ...
            PacketType::Nack(msg) => {
                ...
                match result {
                    None => {
                        println!(
                            "{} {:?}",
                            "Received an NAck but I can't trace back the number to any packet "
                                .white()
                                .bold()
                                .on_blue(),
                            msg
                    ...
                    Some(ack_value) => {
                        if let Some(pos) = ack_value
                            .iter()
                            .position(|f| f.fragment_index == msg.fragment_index)
                        {
            ...
            PacketType::Ack(msg) => {
                ...
                match result {
                    None => {
                        println!("{} {:?}","SERVER --> Received an Ack but I can't trace back the number to any packet ".white().bold().on_blue(), msg)
                    }
                    Some(ack_value) => {
                        /*
                        ...
                        if let Some(pos) = ack_value
                            .iter()
                            .position(|f| f.fragment_index == msg.fragment_index + 1)
                        {
                            //Send the next packet
                            response.pack_type = MsgFragment(ack_value[pos].clone());
                            self.send_valid_packet(source_id, response);
                        } else {
                            //Check if all the packets are arrived correctly
                            if msg.fragment_index as usize == ack_value.len() - 1 {
                                ...
                                self.paket_ack_manger
                                    .remove(&(source_id, packet.session_id));
                            } else {
                                println!(
                                    "{}",
                                    "SERVER --> Index of ack not found."
                                        .white()
                                        .bold()
                                        .on_blue()
                                );
            ...
            PacketType::FloodResponse(path) => {
                ...
                //It's not the job of the server to propagate the message is not a drone
            }
            ...
            PacketType::FloodRequest(mut flood_packet) => {
                ...
                if let Some(last) = flood_packet.path_trace.last() {
                    ...
                    self.packet_send
                        .get(&previous_neighbour)
                        .expect("Error while getting neighbor")
                        .send(response.clone())
                        .expect("Server: Error while sending FloodResponse"); //TODO do match case
                                                                              //}
                } else {
                    panic!(
                        "{} {}",
    ...
    fn sendflod_request(&self) {
        ...
        for (node, sender) in self.packet_send.iter() {
            println!(
                "{} {:?}",
                "Sending flood request to node ".white().bold().on_blue(),
                node
            );
    ...
    fn send_packet(&self, dest_id: NodeId, mut packet: Packet) {
        ...
        match NewWork::bfs_shortest_path(&self.graph, self.id, dest_id) {
            ...
            None => {
                print!(
                    "{}",
                    "Error not found a valid path to follow"
                        .white()
                        .bold()
                        .on_blue()
                )
